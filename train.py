...
parser.add_argument('--config', type=str, default='configs/chiller-train-config_ODA.yaml',
                    help='/path/to/config/file')

parser.add_argument('--source_data', type=str,
                    default='./data_chiller/ODA/source_40_obda.csv',
                    help='path to source list')
parser.add_argument('--target_data', type=str,
                    default='./data_chiller/ODA/target_45_obda.csv',
                    help='path to target list')
parser.add_argument('--entropy', type=bool,
                    default=False,
                    help='if use res entropy')
...
parser.add_argument('--res', type=bool,
                    default=True,
                    help='if use res features')
...
source_loader, target_loader, test_loader, target_folder = return_dataset(source_data, target_data,
                                         evaluation_data, batch_size=batch_size, return_id=True, res=args.res, balanced=False)
...
def train():
    ...
    for step in range(conf.train.min_step + 1):
        ...
        label_s = data_s[1]
        img_t = data_t[0]
        img_s, label_s = Variable(img_s.to(device)), Variable(label_s.to(device))
        ...
        if step > 0 and step % conf.test.test_interval == 0:
            acc_o, h_score = test(step, test_loader, logname, n_share, G,
                                  [C1, C2], open=open, entropy=args.entropy, thr=conf.train.thr)

            feat_get(step, G,[C1, C2], source_loader, test_loader, 'record/features')
